...
dom.StateGraph = ->
  ...
  if !@local.nodes?
    ...
    graph.on = false
    graph.show_component_parent_child_relations = false
    graph.show_unused_data = false
    graph.show_data_dependencies = true
    graph.removed_nodes = []
    graph.focus = null
  ...
  counter = [ [pad(num_components,   max_digits), 'reactive components'], \
              [pad(num_data_entries, max_digits), 'state urls'], \
              [pad(num_dependencies, max_digits), 'state dependencies'] ]
  ...
  controls = [ ['parent-child links', 'show_component_parent_child_relations'], \
               ['state dependencies', 'show_data_dependencies'], \
               ['unused state', 'show_unused_data'] ]
  ...
  control_button = (label, prop) -> 
    ...
    SPAN 
      style: 
        borderRadius: 8
        backgroundColor: if active then state_graph_color else 'transparent'
        fontSize: 14
        fontFamily: 'sans-serif'
        padding: '6px 14px'
        color: if active then "white" else '#777'
        cursor: 'pointer'
        marginRight: 12
  ...
  DIV 
    ...
    DIV 
      style: 
        ...
        left: 0
        padding: 2
        backgroundColor: state_graph_color
        color: 'white'

      for [num, label], idx in counter
        SPAN 
          style: {}
    ...
    DIV 
      ...
      DIV null,
        BUTTON 
          ...
          onClick: => 
            @local.show_instructions = !@local.show_instructions
            save @local
          if @local.show_instructions
            'hide instructions'
          else 
            'show instructions'
      ...
      if focus.is_key
        DIV 
          ...
          DIV 
            ...
            for own k,v of bus.cache[focus.name] #don't call fetch b/c we don't want dependency
              text = pretty_print v
              DIV
                style: 
                  maxWidth: 230
...
refresh_data = (nodes, links) ->
  ...
  for own k,v of bus.components
    ...
    fetched_keys = v.render.fetched_keys
    for keys, busss of fetched_keys
      keys = JSON.parse(keys)
      for dep in keys when dep.substring(0,3) != 'bus' 
        key = ["_#{dep}", k]
        if !(key of links)
          new_links[ key ] = 1

    ...
    DIV
      ...
      DIV 
        ...
        for post,idx in posts
          continue if loc.show_only && (post.key or post) != loc.show_only
          if !loc.show_only && idx > @local.items_to_render
            break 
          else
            THREAD 
              post: post
...
dom.POST = -> 
  ...
  @drawShareableUrl = => 
    DIV 
      ...
      IMG
        src: 'https://f.consider.it/link.png'
    ...
    DIV 
      ...
      for sel in slidergrams
        SELECTION 
          ...
          slidergram_height: SLIDERGRAM_HEIGHT
          one_sided: true
          style: 
            left: 620
        ...
        DIV 
          ...
          if @local.hover_channel
            ...
            if current_channel == 'considerit'
              ...
              items.push 
                name: 'backlog'
                url: 'https://slider.chat/considerit'
...
delete_post = (key_or_object) -> del key_or_object
...
window.onload = ->  
  ...
  if hash
    ...
    loc.show_only = "/post/#{loc.seek_to_hash}"
    save loc 
...
dom.SLIDER_LABEL = ->
  ...
  text_style = defaults {}, @props.style,
    fontSize: 22
    fontWeight: 300
  ...
  DIV 
    style:
      ...
      #left: 6
      marginRight: 6
      top: 3 #@props.height - size.height / 2 - 1 #-size.height - 1
      #width: if @local.hovering || local_sldr.editing_label then 2500
      zIndex: if local_sldr.editing_label || @local.show_menu then 999
      display: 'inline-block' 
      verticalAlign: 'top'
